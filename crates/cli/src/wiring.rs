...
// PACK-GAP-CLOSURE P1 (§2.8): the composition-root pack wiring (ONE rescanned pack
...
pub enum CliWiringError {
    ...
    ConfigTree(String),
    /// PACK-GAP-CLOSURE P1 (§2.8): the boot-time pack wiring failed — the packs dir
...
pub struct WiringHandles {
    ...
    /// PACK-GAP-CLOSURE P1 (§2.8): the composition-root pack wiring — the ONE
...
/// PACK-GAP-CLOSURE P1 (§2.8): [`materialize_config_tree`] over an explicit
...
async fn wire_capabilities_inner(
    ...
    // PACK-GAP-CLOSURE P1 (§2.8): ONE pack registry rescanned from
    ...
    // is fine.
    // PACK-GAP-CLOSURE P1 (§2.8): the same augment ALSO installs the composition-root
    ...
    if let Some(tree) = agent_tree.as_ref() {
        // PACK-GAP-CLOSURE P1 (§2.8): the chained resolver — a guest `spawn-child`
