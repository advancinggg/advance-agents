...
fn build_pack_with_all_10_provides(root: &Path, name: &str, version: &str) -> PathBuf {
    ...
    // PACK-GAP-CLOSURE P2 (§3.1): `mcp-servers/{name}.yaml` now has a schema
    ...
    // PACK-GAP-CLOSURE P2 (§3.1): extensions are parsed structurally — a real
...
async fn t60_materialize_channel_adapter_is_explicitly_unsupported() {
    // PACK-GAP-CLOSURE P2 (§3.1 decision): cap-channel has no path-loaded
...
async fn t63_merge_meta_schema_extension_is_a_structured_single_document_merge() {
    // PACK-GAP-CLOSURE P2 (§3.1): the Slice C `---`-separated append (which
