# guest-rust-j01-skeleton

/dev Slice BS-3 (2026-06-03) — J-01 system-acceptance **skeleton guest** fixture.
...
`agent-fs` host fn — the single load-bearing host call the system-acceptance harness
witnesses (real `fs.write` → cap-fs → git `CommitType::Turn` commit). `run` returns
`Completed`, echoing a received `trigger-context` (when present) into the output as
`event_type|chain_id|depth` (sched-harvest 1B — the SYS-AC-101 populated-context
witness leg; a `None` context keeps `output: None`, the pre-1B shape).

The committed `../guest-rust-j01-skeleton.core.wasm` artifact is wrapped to a
...
5. Copy artifact: `cp target/wasm32-unknown-unknown/release/guest_rust_j01_skeleton.wasm ../guest-rust-j01-skeleton.core.wasm`.
6. Verify size < 500 KiB: `wc -c ../guest-rust-j01-skeleton.core.wasm`.
